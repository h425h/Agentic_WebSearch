...
// Thread-safe: cachedConnections is a ConcurrentHashMap of ConcurrentLinkedDeque,
// so multiple threads can safely take/return cached sockets.
...
public class HTTP {
  ...
  private static final Map<String, Deque<Socket>> cachedConnections = new ConcurrentHashMap<>();
  ...
  public static Response doRequestWithTimeout(String method, String urlArg, byte uploadOrNull[], int timeoutMillis, boolean isHeadRequest) throws IOException {
    ...
    while (true) {
      ...
      Socket sock = null;
      Deque<Socket> q = cachedConnections.get(serverID);
      if (q != null) {
        sock = q.pollFirst();
        if (sock != null) usingCached = true;
      }
      if (sock == null) {
...
//  System.out.println((usingCached ? "[CACHED]" : "[FRESH]")+request);
      ...
      cachedConnections.computeIfAbsent(serverID, k -> new ConcurrentLinkedDeque<>())
                       .add(sock);
