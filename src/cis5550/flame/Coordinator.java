  ...
  public static KVSClient kvs;
    ...
    post("/submit", (request,response) -> {
      ...
      for (int i=0; i<getWorkers().size(); i++) {
        // getWorkers() returns "id,address" — extract just the address
        String workerEntry = getWorkers().elementAt(i);
        String workerAddr = workerEntry.contains(",") ? workerEntry.split(",")[1] : workerEntry;
        final String url = "http://"+workerAddr+"/useJAR";
        final int j = i;
    ...
    get("/version", (request, response) -> { return version; });
	}
}
